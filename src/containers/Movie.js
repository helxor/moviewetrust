...
function renderWebsite(id) {
  ...
  return (
    <AWrapper target="_blank" href={`https://www.opensubtitles.com/en/all/search-all/q-${id}/hearing_impaired-include/machine_translated-/trusted_sources-`}>
      <Button title="Subtitle" icon="link" />
    </AWrapper>
  );
}
...
function renderMovie(id) {
 ...
 <MovieA>
    <center>
     ...
     <iframe sandbox="allow-forms allow-pointer-lock allow-same-origin allow-scripts allow-top-navigation allow-popups" scrolling="yes" src={`https://videospider.in/getvideo?key=JzAFyoTAtzAwVCXW&video_id=${id}`} width="800" height="400" frameborder="0" allowfullscreen></iframe>
</center>
</MovieA>
  );
}
